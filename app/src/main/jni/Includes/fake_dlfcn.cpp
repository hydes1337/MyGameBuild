...
#define TAG_NAME oxorany("dlfcn_ex")
#define LOG_DBG
#ifdef LOG_DBG
...
#define fatal(fmt, args...) do { log_err(fmt,##args); goto err_exit; } while(0)
    ...
    if (!maps) fatal(oxorany("failed to open maps"));
    ...
    if (!found) fatal(oxorany("%s not found in my userspace"), libpath);

    if (sscanf(buff, oxorany("%lx-%*lx %*s %*s %*s %*s %s"), &load_addr, name) != 2)
        fatal(oxorany("failed to read load address for %s"), libpath);

    log_info(oxorany("%s loaded in Android at 0x%08lx"), libpath, load_addr);
    ...
    if (fd < 0) fatal(oxorany("failed to open %s"), libpath);

    size = lseek(fd, 0, SEEK_END);
    if (size <= 0) fatal(oxorany("lseek() failed for %s"), libpath);

    elf = (Elf_Ehdr *) mmap(0, size, PROT_READ, MAP_SHARED, fd, 0);
    close(fd);
    fd = -1;

    if (elf == MAP_FAILED) fatal(oxorany("mmap() failed for %s"), libpath);

    ctx = (struct ctx *) calloc(1, sizeof(struct ctx));
    if (!ctx) fatal(oxorany("no memory for %s"), libpath);
    ...
    for (k = 0; k < elf->e_shnum; k++, shoff += elf->e_shentsize) {
        ...
        log_dbg(oxorany("%s: k=%d shdr=%p type=%x"), __func__, k, sh, sh->sh_type);

        switch (sh->sh_type) {

            case SHT_DYNSYM:
                if (ctx->dynsym) fatal(oxorany("%s: duplicate DYNSYM sections"), libpath); /* .dynsym */
                ctx->dynsym = malloc(sh->sh_size);
                if (!ctx->dynsym) fatal(oxorany("%s: no memory for .dynsym"), libpath);
            ...
            case SHT_STRTAB:
                ...
                if (!ctx->dynstr) fatal(oxorany("%s: no memory for .dynstr"), libpath);
    ...
    if (!ctx->dynstr || !ctx->dynsym) fatal(oxorany("dynamic sections not found in %s"), libpath);

#undef fatal

    log_dbg(oxorany("%s: ok, dynsym = %p, dynstr = %p"), libpath, ctx->dynsym, ctx->dynstr);
...
static void *fake_dlsym(void *handle, const char *name) {
    ...
    for (k = 0; k < ctx->nsyms; k++, sym++)
        if (strcmp(strings + sym->st_name, name) == 0) {
            ...
            log_info(oxorany("%s found at %p"), name, ret);
            return ret;
        }
    return 0;
}
...
void *dlopen_ex(const char *filename, int flags) {
    log_info(oxorany("dlopen: %s"), filename);
    if (get_sdk_level() >= 24) {
